...
class MFD_Zoning:
    ...
    def clean_zone_polygone(self, polygon_path):
        """
        Clean zone polygons from the clusters and save as a GeoJson file.
        """
        # ポリゴンデータを読み込む
        polygons = gpd.read_file(polygon_path)

        # インナーリングを削除した新しいジオメトリを作成
        for idx, feature in polygons.iterrows():
            ...
            if geom.geom_type == "MultiPolygon":
                ...
                for polygon in geom.geoms:
                    new_polygons.append(Polygon(polygon.exterior))  # インナーリングを削除
                new_geom = MultiPolygon(new_polygons)

            # 通常のポリゴン
            elif geom.geom_type == "Polygon":
                new_geom = Polygon(geom.exterior)  # インナーリングを削除
            
            else:
                new_geom = geom  # その他のタイプはそのまま
    ...
    def fit_scaled_mfd_per_cluster(self, flow_output_dir_list, show=False, suffix=""):
        ...
        flow_output_dir_list : list
            データが保存されているディレクトリのパスのリスト
        show : bool, optional
            結果を表示するかどうか (デフォルトは False)
        suffix : str, optional
            ファイル名に付加するサフィックス (デフォルトは "")
        """
        def scaled_mfd_model(N, alpha, beta, A_unit, B_unit, C_unit):
            """
            スケールされたMFDモデルを計算する関数
            """
            N_scaled = N / beta
    ...
    def parking_success_rate(self, output_dir_list, show=False, mono=False, edge_data_use=False, is_penalty=True):
        ...
        # 残差関数：本来の残差に加え、制約違反 (y_pred > search_veh) に対して大きなペナルティを加える
        def residuals(params, X, y, penalty_weight=1e5):
            y_pred = model_func(params, X)
            # 観測値との誤差
            res_main = y_pred - y
            # 制約：y_pred <= search_veh を満たすよう、違反部分 (y_pred - search_veh > 0) にペナルティを追加
        ...
        if mono:
            ...
            for i in range(len(self.clusters)):
                ...
                alphas_contour = (contour_levels - Z.min()) / (Z.max() - Z.min())  # 正規化された透明度
            ...
            cbar.ax.yaxis.set_tick_params(labelsize=16)  # ここでフォントサイズを設定
            ...
            for i in range(len(self.clusters)):
                ...
                fig.add_trace(go.Isosurface(
                    ...
                    isomin=contour_min,
                    isomax=contour_max,
                    surface_count=5,      # 表示する等値面の数（適宜変更）
                    #surface_fill=0.05,
                    colorscale="RdBu_r",
                    opacity=0.6,
                    caps=dict(x_show=False, y_show=False, z_show=False),
                    showscale=False        # カラーバーを表示する場合は True
                ))
                ...
                fig.add_trace(go.Scatter3d(
                    ...
                    marker=dict(
                        size=5,
                        color=y,             # parking_success_rate の値に基づいて色分け
                        colorscale="RdBu_r",
                        opacity=0.8,
                ...
                # グラフの出力（show が True なら表示、False なら画像として保存）
                if show:
                    #fig.show()
                    pass
                else:
