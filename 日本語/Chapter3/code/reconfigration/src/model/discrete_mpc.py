...
class CrossEntropy():
    ...
    def __init__(self, params, initial_graph, time_steps, seq_interval, control_start_time, control_end_time, prediction_end_time, noise_scale, output_path='../../output/mpc/mpc'):
        self.params = params
        self.alpha = 0.7
        self.reconf_end_time = 12 * 60
        self.control_init_time = control_start_time // self.params.sampling_time
        self.sim = MFD_Dynamics(params, output_path=None)
        # self.simをcontrol_start_timeまで進める
        for _ in range(self.sim.sim_start_step, self.control_init_time):
    ...
    def prepare_noise(self):
        np.random.seed(42)
        steps = self.sim.sim_end_step - self.control_init_time
        ...
        while converged == False:
            ...
            for t in range(1, self.time_steps):
                ...
                for idx, ind in enumerate(group):
                    ...
                    while n_sample < target_count:
                        # ランダムな制御変数を生成
                        tmp = (np.random.rand(10**3, self.num_links) < p[(t*self.num_links):((t+1)*self.num_links)]).astype(int)
                ...
                if len(invalid_idx) > 0:
                    ...
                    for idx in invalid_idx:
                        population[indices == idx, :] = np.tile(population[indices == random.choice(valid_indices), :][0,:], (np.sum(indices == idx), 1))
            ...
            tasks = [
                self.evaluate_candidate.remote(
                    ...
                    self.link_indices, self.control_start_time, self.control_end_time, self.prediction_end_time, self.time_steps, self.sim
    ...
    def sim_mpc_step(self, best_control_sequence):
        ...
        for t in range(self.control_start_time, self.control_start_time + self.seq_interval):
            # プラントに摂動を与える
            sim.step_simulation(self.noise_x[t - self.control_init_time], self.noise_x_background[t - self.control_init_time])
    ...
    def sim_no_policy(self):
        ...
        sim = copy.deepcopy(self.sim)
        sim.output_path = f'../../output/mfd_dynamics/green_{self.params.green_split}/{self.params.demand}/{self.params.demand_variation}/back{self.params.background_ratio}/{int(sim.params.simulation_start_time/60)}_{int(sim.params.simulation_end_time/60)}_mpc_no_policy'
        #sim.reset()
        for t in range(self.control_init_time, sim.sim_end_step):
            sim.step_simulation(self.noise_x[t - self.control_init_time], self.noise_x_background[t - self.control_init_time])
        ...
        print(f"Throughput with no policy: {np.array(sim.throughput_list[self.reconf_end_time:]).sum() + np.array(sim.throughput_background_list[self.reconf_end_time:]).sum()}")
        N_p_back = sim.xs_background[self.reconf_end_time][self.params.num_zones**2 : ]
        print("ATT with no policy: ", np.array(sim.ttt_list[:self.reconf_end_time]).sum() / N_p_back.sum())
    
    def sim_best_policy(self, best_control_sequence: np.ndarray):
        sim = MFD_Dynamics(self.params, output_path=None)
        # simをcontrol_start_timeまで進める
        for _ in range(sim.sim_start_step, self.control_init_time):
            sim.step_simulation()
        ...
        self.params.max_boundary_capacity = copy.deepcopy(self.original_capacity)
        for t in range(self.control_init_time, sim.sim_end_step):
            if ((sim.step % self.seq_interval) == 0) and (sim.step < (self.control_init_time + best_control_sequence.shape[0]*60)):
                print(t)
                # キャパシティを元に戻す
                sim.params.max_boundary_capacity = copy.deepcopy(self.original_capacity)
                # コントラフローのリンクを取得
                contraflow_idx = best_control_sequence[(sim.step // self.seq_interval) - (self.control_init_time // self.seq_interval),:]
        ...
        print(f"Throughput with best policy: {np.array(sim.throughput_list[self.reconf_end_time:]).sum() + np.array(sim.throughput_background_list[self.reconf_end_time:]).sum()}")
        N_p_back = sim.xs_background[self.reconf_end_time][self.params.num_zones**2 : ]
        print("ATT with best policy: ", np.array(sim.ttt_list[:self.reconf_end_time]).sum() / N_p_back.sum())
        self.reconf.draw_sequence(best_control_sequence_graph)
        

if __name__ == '__main__':
    noise_scale = 0 #1/20 # 毎秒の状態変数xの摂動のスケール．noise_scale*xが摂動の標準偏差．
    ...
    elite_count, N_min, N_max, N_unit, d, max_cpu = 10, 400, 1000, 300, 4, 50
    #elite_count, N_min, N_max, N_unit, d, max_cpu = 5, 10, 20, 10, 2, 2
    
    # MPC
    contraflow_graphs = []
    params = Parameters()
    ce = CrossEntropy(params, initial_graph, time_steps, seq_interval, mpc_start_time, mpc_start_time+control_horizon, mpc_start_time+prediction_horizon, noise_scale)
    ce.sim_no_policy()
    
    for control_start_time in tqdm(range(mpc_start_time, mpc_end_time, seq_interval)):
        start = time.time()
        ce.control_start_time= control_start_time
        if control_start_time + control_horizon > mpc_end_time:
            ce.control_end_time = mpc_end_time
        else:
            ce.control_end_time = control_start_time + control_horizon # min. 遷移の終了時間
        if control_start_time + prediction_horizon > mpc_end_time:
            ce.prediction_end_time = mpc_end_time
        else:
            ce.prediction_end_time = control_start_time + prediction_horizon # min. 遷移の終了時間
