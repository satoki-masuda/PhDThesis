"""生データを推定・simulation 向けの中間データへ整形する関数群。"""
...
def read_pt_data(start_year, end_year, zoning_type):
    """PT 個票データから世帯単位の転居履歴パネルを作成または再利用する。"""
    base_path = Path(__file__).resolve().parent.parent  # model/ から1階層上へ
    ...
    df1 = df1[(df1["39_■5_居住年数_①回答区分"]!=9) & (df1["41_■6_以前のお住まい_①回答区分"]!=2) & (df1["41_■6_以前のお住まい_①回答区分"]!=9)].reset_index(drop=True) # 居住年数不明・転居前住所不明、外国を除外
    df1 = df1[(df1["37_■5_所有関係_①回答区分"]==1)].reset_index(drop=True) # 持ち家に絞る
    ...
    # 見つからなかった世帯IDをdf_poolから削除
    not_found = df1[df1["世帯主年齢"].isna()]["5_整理番号_市町村・ロット・SEQ"].unique()
    if len(not_found) > 0:
        print(f"以下の世帯IDが見つかりませんでした: {not_found}")
...
def read_pop_data(start_year, end_year, zoning_type):
    """住民基本台帳から年次・ゾーン別人口辞書を作成または再利用する。"""
    base_path = Path(__file__).resolve().parent.parent  # model/ から1階層上へ
    folder_path = os.path.join(base_path, f"data/processed/{zoning_type}")
    ...
    for year in range(start_year, end_year+1):
        ...
        if year == 2018:
            pop = pd.read_excel(base_path / f"data/raw/町丁別住民基本台帳登録人口/2018.xls", header=2, usecols=usecols, sheet_name=11)
        elif year >= 2023:
            pop = pd.read_excel(base_path / f"data/raw/町丁別住民基本台帳登録人口/{year}.xlsx", header=2, usecols=usecols, sheet_name="1月1日")
        else:
            pop = pd.read_excel(base_path / f"data/raw/町丁別住民基本台帳登録人口/{year}.xls", header=2, usecols=usecols, sheet_name="1月1日")
        ...
        print(f"{year}年の欠損ゾーン: {sorted(list(zero_zones))}")
        for zone in zero_zones:
            pop_dict[year]['人口'][zone] = 0
            pop_dict[year]['世帯数'][zone] = 0
        
    
    print(f"マッピングできなかった町名: {sorted(list(set(na_list)))}")
    print(f"地区数: {[len(pop_dict[year]['人口']) for year in range(start_year, end_year+1)]}")
    # JSONファイルとして保存
    if not os.path.exists(folder_path):
        os.makedirs(folder_path)
...
def read_building_data(start_year, end_year, zoning_type):
    """建築確認データから年次・ゾーン別の新築面積を作成または再利用する。"""
    base_path = Path(__file__).resolve().parent.parent  # model/ から1階層上へ
    folder_path = os.path.join(base_path, f"data/processed/{zoning_type}")
    ...
    af1 = af1.loc[(af1["種別"]=="新築"), :].reset_index(drop=True)  # 新築住宅のみを抽出
    ...
    af2 = af2.loc[(af2["工事種別"]=="新築"), :].reset_index(drop=True)  # 新築住宅のみを抽出
    ...
    # building_dictのキーをJSON対応の文字列に変換
    json_building_dict = {}
...
def point_to_zone(gdf:gpd.GeoDataFrame, zoning):
    """点データに対応する `選択ゾーン` を付与する。"""
    # gpdの各行のpointがどのゾーンに属するかを判定する列を追加
...
def convert_jp_year(x):
    """和暦文字列を西暦ベースの `YYYYMMDD` 文字列に変換する。"""
    if pd.isna(x):
        return x
    x = str(x)
    if x.startswith('H'):
...
def read_los_data(start_year, end_year, zoning_type):
    ...
    # dict_bus, dict_shinai, dict_kougai, dict_totalを結合
    dict_los = {}
    for year in range(start_year, end_year + 1):
        dict_los[year] = {}
...
def read_obs_share_data(start_year, end_year, zoning, zoning_type):
    ...
    base_path = Path(__file__).resolve().parent.parent  # model/ から1階層上へ
    data_inside = pd.read_excel(base_path / "data/raw/地区別社会動態_地区内移動_地区外移動/data.xlsx", sheet_name="地区内転居入")
    data_move_in = pd.read_excel(base_path / "data/raw/地区別社会動態_地区内移動_地区外移動/data.xlsx", sheet_name="地区外転入")
    # zoning["ゾーン名"].astype(str).tolist()に含まれるゾーンのみ抽出
    ...
    for i, row_i in choice_zone.iterrows():
        for j, row_j in choice_zone.iterrows():
            ...
            distance_records.append({
                "zone_1": row_i["選択ゾーン"],
                "zone_2": row_j["選択ゾーン"],
                "distance_km": round(dist, 2)/1000,  # mからkmに変換
            })
...
def read_move_data(zoning):
    """ゾーン別の転居率・市外流出率・流入人口を読み込む。"""
    base_path = Path(__file__).resolve().parent.parent  # model/ から1階層上へ
    move_ratio = pd.read_excel(base_path / "data/raw/地区別社会動態_地区内移動_地区外移動/data.xlsx", sheet_name="地区内転居出率")
    move_ratio = move_ratio.loc[move_ratio["ゾーン"].isin(zoning["ゾーン名"].astype(str).tolist()), :].drop(columns=["ゾーン", "期間中平均移動率"]).values
    exiting_ratio = pd.read_excel(base_path / "data/raw/地区別社会動態_地区内移動_地区外移動/data.xlsx", sheet_name="地区外転出率")
    exiting_ratio = exiting_ratio.loc[exiting_ratio["ゾーン"].isin(zoning["ゾーン名"].astype(str).tolist()), :].drop(columns=["ゾーン", "期間中平均移動率"]).values
    inflow_pop = pd.read_excel(base_path / "data/raw/地区別社会動態_地区内移動_地区外移動/data.xlsx", sheet_name="地区外転入")
