"""Parameter container that gathers all preprocessed inputs for Chapter 3 models."""
...
class Parameters:
    """Bundle simulation inputs, calibrated coefficients, and routing probabilities."""

    def __init__(self, demand: str = "demand_36h", demand_variation: str = "93_max", background_ratio: float = 1.0):
        """Load calibrated data and construct the zone-level parameter set."""
        #mfd_dynamics
        ...
        self.demand = demand
        self.demand_variation = demand_variation #"67_min", #"75_0.25", #"46_mean" #"16_0.75" #"38_0.8", #"78_0.85", #"71_0.9", #"53_0.95", #"93_max", # "normal"
        self.background_ratio = background_ratio
        self.contra_ratio = 2.0
        self.green_split = 0.5
        self.simulation_start_time = 0 * 60 #シミュレーションの開始時間, min
        self.simulation_end_time = 24 * 60 #シミュレーションの終了時間, min # 48 * 60
        ...
        self.path_update_interval = 60  # 経路の更新周期, min
    ...
    def k_shortest_theta(self):
        """Compute route-choice probabilities from the enumerated k-shortest paths."""
        theta = np.zeros((self.num_zones, self.num_zones, self.num_zones))
    ...
    def load_od_matrix(self, od_file):
        """Read a processed OD CSV into the 3D demand tensor used by the simulator."""
    ...
    def plot_od(self, polygon_path):
        """Visualize processed OD demand on top of the zone polygon geometry."""
        '''
